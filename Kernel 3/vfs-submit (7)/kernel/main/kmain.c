...
{
    int status;
    proc_t *test_proc = proc_create("sunghan_proc");
    kthread_t *test_thread = kthread_create(test_proc, sunghan_test, NULL, NULL);
...
{
    int status;
    proc_t *test_proc = proc_create("sunghan_deadlock_proc");
    kthread_t *test_thread = kthread_create(test_proc, sunghan_deadlock_test, NULL, NULL);
    sched_make_runnable(test_thread);
    do_waitpid(test_proc->p_pid, 0, &status);
...
{
    int status;
    proc_t *test_proc = proc_create("faber_thread_proc");
    kthread_t *test_thread = kthread_create(test_proc, faber_thread_test, NULL, NULL);
    sched_make_runnable(test_thread);
    do_waitpid(test_proc->p_pid, 0, &status);
    return 0;
}
...
int my_vfs_test(kshell_t *kshell, int argc, char**argv )
{
        ...
        kthread_t *vfs_thread = kthread_create(vfs_proc, vfstest_main, 1, NULL);

        sched_make_runnable(vfs_thread);

        int status;         
	do_waitpid(vfs_proc->p_pid, 0, &status);
        return 0;
}
...
bootstrap(int arg1, void *arg2)
{
        ...
        dbgq(DBG_TEST, "SIGNATURE: 53616c7465645f5f4177be8aa7d3122fc93cbfbd16ce765d9223ad00f8656c1b85e736325155d6201c7182c8aa65dcbc\n");
        /* necessary to finalize page table information */
        pt_template_init();

        proc_t* idleproc = proc_create("idleproc");
        KASSERT(NULL != idleproc);
	dbg(DBG_PRINT, "(GRADING1A)\n");

        KASSERT(idleproc->p_pid == 0);
	dbg(DBG_PRINT, "(GRADING1A)\n");
        ...
        kthread_t *new_thread = kthread_create(idleproc, idleproc_run, NULL, NULL);
        curthr = new_thread;
        KASSERT(NULL != curthr);
        dbg(DBG_PRINT, "(GRADING1A 1.a)\n");
...
{
    proc_t *new_proc = proc_create("init process");

    KASSERT(NULL != new_proc);
    dbg(DBG_PRINT, "(GRADING1A 1.b)\n");

    KASSERT(PID_INIT == new_proc->p_pid);
    dbg(DBG_PRINT, "(GRADING1A 1.b)\n");

    kthread_t *thr = kthread_create(new_proc, initproc_run, NULL, NULL);
    KASSERT(NULL != thr);
    dbg(DBG_PRINT, "(GRADING1A 1.b)\n");
