...
proc_cleanup(int status)
{
       ...
       if(curproc->p_pproc->p_wait.tq_size > 0)       
        ...
        {
            proc_t *processchild;
            list_iterate_begin(&curproc->p_children, processchild, proc_t, p_child_link) 
            { 
                    processchild->p_pproc = proc_initproc;
                    list_insert_tail(&proc_initproc->p_children,&processchild->p_child_link);
                    list_remove(&processchild->p_child_link);
		    dbg(DBG_PRINT, "(GRADING1A)\n");             
            } list_iterate_end();
...
proc_kill_all()
{
    ...
    list_iterate_begin(&_proc_list, p, proc_t, p_list_link)
    {
        if(p != curproc && p->p_pid != PID_IDLE && p->p_pid != PID_INIT)
        {
            if(p->p_pproc->p_pid != PID_IDLE) 
            {
...
* is the child of curproc
*/
int
is_child_of_process(pid_t pid)
{
	proc_t *p;
...
/*
* find the child process with given pid
*/
proc_t* find_process_with_pid(pid_t pid)
{
	proc_t *p;
	list_iterate_begin(&curproc->p_children, p , proc_t, p_child_link){
...
/*
* find any child process that exited
*/
proc_t* find_any_exited_process()
{
	proc_t *p;
	list_iterate_begin(&curproc->p_children, p , proc_t, p_child_link){
...
*/
void
kill_process(proc_t *proc)
{
	list_t temp_threads = proc->p_threads;
	
	list_link_t *temp_dest_thread = temp_threads.l_next;

	kthread_t* dest_thread = (kthread_t*) list_item(temp_dest_thread, kthread_t, kt_plink);

	kthread_destroy(dest_thread);

	KASSERT(KT_EXITED == dest_thread->kt_state);
	dbg(DBG_PRINT,"(GRADING1A)\n");

	list_remove(&proc->p_child_link);
    
	slab_obj_free(proc_allocator, (void *) proc);
}

/*
* Dispose the given process
*/
pid_t
dispose_process(pid_t pid, int *status)
{
	proc_t* child_proc = find_process_with_pid(pid);
	...
	*status = child_proc->p_status;
	pid_t child_exit_pid = child_proc->p_pid;

	KASSERT(pid == -1 || child_proc->p_pid == pid);
	dbg(DBG_PRINT, "(GRADING1A 2.c)\n");

	kill_process(child_proc);
	dbg(DBG_PRINT,"(GRADING1A)\n");
	return child_exit_pid;
	
}

/*
* Dispose any of the exited child process
*/
pid_t
dispose_any_process(int *status)
{
	proc_t* child_proc = find_any_exited_process();

	while(NULL == child_proc) {
		sched_sleep_on(&curproc->p_wait);
		child_proc = find_any_exited_process();
	}
...
do_waitpid(pid_t pid, int options, int *status)
{
	...
	else if(pid > 0 && is_child_of_process(pid) == 1)
	{
		dbg(DBG_PRINT, "(GRADING1C)\n");
		return dispose_process(pid, status);
	}
	else if(((int)pid) == -1)
	{
		dbg(DBG_PRINT, "(GRADING1C)\n");
		return dispose_any_process(status);	
	}
	else
	{
		dbg(DBG_PRINT,"(GRADING1C)\n");
