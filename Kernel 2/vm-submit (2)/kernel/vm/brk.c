...
{        
        ...
        if (addr < curproc->p_start_brk || addr >= (void*)USER_MEM_HIGH) 
	{
        ...
        uint32_t start_vfn = ADDR_TO_PN(PAGE_ALIGN_DOWN((uint32_t) curproc->p_start_brk));
        uint32_t end_vfn  = ADDR_TO_PN(PAGE_ALIGN_DOWN((uint32_t)addr-1))+1;


        vmarea_t *vmarea_temp_1 = vmmap_lookup(curproc->p_vmmap, start_vfn);

        /*if (vmarea_temp_1 == NULL) 
	{
dbg(DBG_TEMP, " -------------------------------------------------------------------------------- CHECK  190\n");
		dbg(DBG_PRINT, "(GRADING3D 1)\n");
                return -ENOMEM;
        }*/

        vmarea_t *vmarea_temp_2 = list_item((vmarea_temp_1->vma_plink.l_next), vmarea_t, vma_plink);


        if(end_vfn > vmarea_temp_2->vma_start) 
	{
		dbg(DBG_PRINT, "(GRADING3D 2)\n");
                return -ENOMEM;
        }

        vmarea_temp_1->vma_end = end_vfn;
        curproc->p_brk = addr;
        *ret = addr;
	dbg(DBG_PRINT, "(GRADING3D 1)\n");
        return 0;
