...
sys_read(read_args_t *arg)
{
	...
	temp = copy_from_user(&kern_args, arg, sizeof(kern_args));
	KASSERT(temp >= 0);
	dbg(DBG_PRINT, "(GRADING3B 1)\n");

	cond = ((uint32_t)PAGE_ALIGN_UP(kern_args.nbytes) / PAGE_SIZE);
	for (index = 0; index < cond; index++)
	{
	    	buffer = page_alloc();
		...
		temp = (int)kern_args.nbytes - (int)(index * PAGE_SIZE);
		KASSERT(temp >= 0);
        	dbg(DBG_PRINT, "(GRADING3B 1)\n");

	   	retVal = do_read(kern_args.fd, buffer, (kern_args.nbytes - index * PAGE_SIZE));
	   	if (retVal < 0) 
		{
		...
		{
			temp = copy_to_user((void*)((uint32_t)kern_args.buf + index * PAGE_SIZE), buffer, retVal);
			KASSERT(temp >= 0);
			dbg(DBG_PRINT, "(GRADING3B 1)\n");
...
sys_write(write_args_t *arg)
{
	...
	int retVal_total = 0;
	int temp = 0;
	size_t index = 0, cond = 0, temp_len = 0;
	void *buffer;
    	...
    	for (index = 0; index < cond; index++)
	{
		...
		dbg(DBG_PRINT, "(GRADING3B 1)\n");
        	...
        	if ((index + 1) * PAGE_SIZE > kern_args.nbytes)
		{
            		temp_len = kern_args.nbytes - index * PAGE_SIZE;
        		dbg(DBG_PRINT, "(GRADING3B 1)\n");
        	}
		else
		{
		...
		temp = copy_from_user(buffer, (void*)((uint32_t)kern_args.buf + index * PAGE_SIZE), temp_len);
		KASSERT(temp >=0);
        	dbg(DBG_PRINT, "(GRADING3B 1)\n");

		retVal = do_write(kern_args.fd, buffer, temp_len);
            	if (retVal >= 0)
		{
                	retVal_total += retVal;
        		dbg(DBG_PRINT, "(GRADING3B 1)\n");
...
sys_getdents(getdents_args_t *arg)
{
	...
	cond = count / (int)sizeof(dirent_t);
	for (int index = 0; index < cond; index++)
	{
		retVal = do_getdent(kern_args.fd, &dir_temp);

		if (retVal > 0) 
		{
			temp = copy_to_user(kern_args.dirp + index, &dir_temp, retVal);
			retVal_total += retVal;
