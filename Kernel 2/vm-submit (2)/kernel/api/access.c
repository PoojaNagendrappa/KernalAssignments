...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
       int vfn = ADDR_TO_PN(vaddr);
		...
		dbg(DBG_TEMP, " -------------------------------------------------------------------------------- CHECK  1\n");
            	return 0;
        }
	dbg(DBG_PRINT, "(GRADING3B 1)\n");
	return perm & vmarea->vma_prot;
}
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
        ...
        uint32_t firstpage = ADDR_TO_PN(avaddr), lastpage = ADDR_TO_PN((uintptr_t)avaddr+len);
		
	if((uintptr_t)((char*)avaddr - len) >= (USER_MEM_HIGH))
	{
		dbg(DBG_PRINT, "(GRADING3D 1)\n");
		return 0;
	}
	...
	{
		if((err = addr_perm(p, PN_TO_ADDR(i), perm)) == 0)
		{
			curthr->kt_errno = -err;
			dbg(DBG_PRINT, "(GRADING3D 1)\n");
			dbg(DBG_TEMP, " -------------------------------------------------------------------------------- CHECK  4\n");
			return 0;
		}
		dbg(DBG_PRINT, "(GRADING3B 1)\n");
	}
