...
do_read(int fd, void *buf, size_t nbytes)
{
	...
	if((rfile->f_mode & FMODE_READ) == 0)
	{
        ...
        int rcount = 0;
 	rcount = rfile->f_vnode->vn_ops->read(rfile->f_vnode, rfile->f_pos, buf, nbytes);
        if(rcount > 0)
	{
                rfile->f_pos += rcount;
		dbg(DBG_PRINT, "(GRADING2B)\n");
...
do_write(int fd, const void *buf, size_t nbytes)
{
	...
	if((wfile->f_mode & FMODE_WRITE) == 0)
        ...
        if((wfile->f_mode & FMODE_APPEND) != 0)
	{
                do_lseek(fd,0,SEEK_END);
		dbg(DBG_PRINT, "(GRADING2B)\n");
        }

        int wcount = wfile->f_vnode->vn_ops->write(wfile->f_vnode, wfile->f_pos, buf, nbytes);
        
	if(wcount > 0)
	{
		wfile->f_pos += wcount;
		KASSERT((S_ISCHR(wfile->f_vnode->vn_mode)) || 
                        (S_ISBLK(wfile->f_vnode->vn_mode)) || 
                        ((S_ISREG(wfile->f_vnode->vn_mode)) && (wfile->f_pos <= wfile->f_vnode->vn_len)));
...
do_mknod(const char *path, int mode, unsigned devid)
{
	...
	vnode_t *lnode;
        ...
        int result = dir_namev(path, &namelen, &name, vfs_root_vn, &dir_vnode);
	...
	KASSERT(dir_vnode->vn_ops->mknod);
...
do_rmdir(const char *path)
{
        ...
        retVal = dir_namev(path, &namelen, &name, NULL, &dir_vnode);
...
do_unlink(const char *path)
{
       	size_t namelen = 0;
        const char* name = NULL;
        vnode_t *dir_vnode = NULL, *lnode = NULL;
        int result = 0;
        ...
        if(result != 0)
	{
        ...
        if((result = lookup(dir_vnode, name, namelen, &lnode)) != 0)
	{
                vput(dir_vnode);
		dbg(DBG_PRINT, "(GRADING2B)\n");
                return result;
...
do_link(const char *from, const char *to)
{
	...
	int result = open_namev(from, 0, &res_vnode, NULL);
	...
	result = dir_namev(to, &namelen, &name, NULL, &lnode);
	if(result < 0)
        {
		vput(res_vnode);
		dbg(DBG_PRINT, "(GRADING2B)\n");
	...
	result = lookup(lnode, name, namelen, &onode);
	if(result == 0)
        {
		vput(res_vnode);
		vput(lnode);
	...
	int retVal = lnode->vn_ops->link(res_vnode, lnode, name, namelen);
	vput(res_vnode);
	vput(lnode);
	dbg(DBG_PRINT, "(GRADING2B)\n");
	return retVal;
