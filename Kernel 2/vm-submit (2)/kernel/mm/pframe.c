...
{
       	pframe_t *page_g = NULL;
	pframe_t *pframe_p = NULL;
	int retVal = 0;
		 
        while (1)
	{
            	page_g = pframe_get_resident(o, pagenum);
		
            	if (page_g != NULL)
		{
                	if (pframe_is_busy(page_g))
			{
                    		sched_sleep_on(&page_g->pf_waitq);
		...
		{
			KASSERT(!pageoutd_needed()); 
			dbg(DBG_PRINT, "(GRADING3D 1)\n");		
					 
                	pframe_p = pframe_alloc(o, pagenum);
			KASSERT(pframe_p != NULL);
			dbg(DBG_PRINT, "(GRADING3D 1)\n");		
					
                	retVal = pframe_fill(pframe_p);
                	dbg(DBG_PRINT, "(GRADING3D 1)\n");
                	if (retVal == 0)
			{
                    		*result = pframe_p;
...
pframe_pin(pframe_t *pf)
{
	...
	dbg(DBG_PRINT, "(GRADING3A 1.b)\n");
        ...
        pframe_t *pageframecount;
        list_iterate_begin(&alloc_list, pageframecount, pframe_t, pf_link)
        {
         	if ( pageframecount == pf) 
          	{ 
              		npinned++;
              		nallocated--;
              		list_remove(&pageframecount->pf_link);
        ...
        list_iterate_begin(&pinned_list, pageframecount, pframe_t, pf_link) 
        {  
          	if ( pageframecount == pf) 
         	{ 
              		pf->pf_pincount++;
